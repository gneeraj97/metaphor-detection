...
def predict(path_to_csv):
    ...
    test_data = pd.read_csv(path_to_csv)

    # Tokenize the 'text' column and create a new column 'sentences'
    test_data['sentences'] = test_data['text'].apply(sent_tokenize)
...
if __name__ == "__main__":
    ...
    parser.add_argument("path_to_csv", type=str, help="Path to the test data CSV file.")
    args = parser.parse_args()

    result_path = predict(args.path_to_csv)
    print(f"Boolean labels saved at in the current working directory as predicted_labels.csv. The column label_boolean is present, similar to the training data")
